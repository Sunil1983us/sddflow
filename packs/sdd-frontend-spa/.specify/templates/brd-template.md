...
> **except Build effort below**, which is never extractable from context.md by design (it
> depends on analyze.md, generated by `/analyze`, which runs AFTER `/validate` in the pipeline
> order: SPECIFY → GATE-1 → VALIDATE → ANALYZE). Write it as plain deferred text, "Pending —
> estimated after /analyze", never as a `[NEEDS CLARIFICATION]` marker — `/checklist`'s CRITICAL
> "no unresolved markers" rule would otherwise flag it as blocking `/validate`, when the
> framework's own designed sequence makes it impossible to resolve before `/validate` runs.
> `/analyze` fills in the real T-shirt size once it estimates effort — see its own "Update BRD
> Build Effort" step.

| Item | Value | Source / Notes |
|---|---|---|
| Delivery scope | {pilot / mvp / full} | manifest.yml |
| Build effort (T-shirt) | Pending — estimated after /analyze | Filled in by /analyze once effort is estimated — see §9 note above; never a [NEEDS CLARIFICATION] marker |
