...
class TestWarnParentLinkFailed:
    def test_attempts_relates_link_as_fallback(self):
        client = MagicMock()
        _warn_parent_link_failed(client, "TEMPT-2", "TEMP-1", "TEMPT", Exception("400 Bad Request"))
    ...
    def test_failure_message_keeps_original_troubleshooting_hint_when_fallback_also_fails(self, capsys):
        client = MagicMock()
    ...
    def test_original_error_always_surfaced(self, capsys):
        client = MagicMock()
        client.link_issues.return_value = None
        _warn_parent_link_failed(client, "TEMPT-2", "TEMP-1", "TEMPT", Exception("cross-project parent rejected"))
