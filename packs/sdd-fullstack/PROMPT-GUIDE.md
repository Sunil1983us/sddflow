...
| `/plan-design` | `/plan-design` | `/plan-design` | Architecture + plan + refine scope docs (both layers) |
...
## /plan-design — Architecture + Plan + Refine Scope Docs
...
REFINE SCOPE-SCALED DOCS (now that design.md exists):
  mvp+: api-spec.md — align with the port/adapter contracts (backend)
        and service layer (frontend) in design.md — single shared contract
  mvp+: component-spec.md, ux-flow.md — align with the frontend component
        tree and state architecture in design.md
  ...
  full: resilience.md — align with design.md integration list and error/
        offline handling strategy (both layers)
  full: investigation.md — align with design.md flows (both layers)
  Re-save each refined doc + its .summary.md.

State: "PLAN-DESIGN complete — review design.md + plan.md (and refined
api-spec/component-spec/ux-flow/data-model/security-design/resilience/
investigation) before PLAN-DESIGN"
