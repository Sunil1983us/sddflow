...
Not sure which pack to use? Use this one — `setup.sh` detects your project type automatically.
...
# OR: manual setup
unzip sdd-universal.zip -d my-project
cd my-project
git init
...
| `sdd config init` (interactive wizard) → `~/.sdd/config.yml` | **Required if using jira/confluence mode** | Before the first `sdd review submit` / `sdd jira` / `sdd confluence` command — `integrations.yml` alone holds no secrets | Atlassian base URL, auth mode, and credential storage choice (OS keychain — recommended — or an environment variable). Never writes a plaintext secret to disk. Can also generate `.specify/integrations.yml` for you in the same run. See HOW-TO-USE.md "Jira & Confluence Integration → One-Time Setup" |
