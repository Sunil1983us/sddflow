...
/change "scope upgrade from pilot to mvp — add api-spec and data-model (living, service-level)"
```
...
2. **Detects current stage** from documents that exist — most live at
   ...
   `data-model.md` are **living documents** shared across every feature in
   this service, at `.specify/service/{doc}.md` — see "Living Documents &
   Cross-Feature Impact" below
...
that other feature's own `srd.md`/`design.md` are never read during a
normal `/change` walk, since the walk is scoped to the feature that
raised the CR.
...
— both living, at `.specify/service/` — and LLD).
...
#    - INCORPORATE api-spec + data-model if this is the first feature in
#      the service to need them (or, if a prior feature already created
#      .specify/service/api-spec.md / data-model.md, EXTEND them via the
