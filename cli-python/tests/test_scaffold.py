...
def fake_pack(tmp_path, monkeypatch):
    ...
    (pack / ".github" / "prompts" / "specify.prompt.md").write_text(
        "v1 specify prompt\n"
    )
    (pack / ".claude" / "commands" / "specify.md").write_text("v1 specify command\n")
    (pack / ".specify" / "templates" / "brd-template.md").write_text(
        "template content\n"
    )
...
def test_scaffold_pack_never_overwrites_existing_files(fake_pack, project):
    ...
    ).read_text() == "locally customized\n"
    assert (
        project / ".claude" / "commands" / "specify.md"
    ).read_text() == "v1 specify command\n"
...
def test_sync_prompts_never_touches_files_outside_synced_dirs(fake_pack, project):
    ...
    assert (
        templates_dir / "brd-template.md"
    ).read_text() == "locally edited template\n"
...
def test_sync_prompts_dry_run_does_not_write_anything(fake_pack, project):
    ...
    assert result["updated"] == [".github/prompts/specify.prompt.md"]
    assert (
        prompts_dir / "specify.prompt.md"
    ).read_text() == "stale v0 content\n"  # unchanged
    assert not (project / ".specify" / ".prompt-sync-backups").exists()
    assert not (
        project / ".claude"
    ).exists()  # the missing "added" file was never written either
