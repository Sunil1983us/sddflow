...
2. **Detects current stage** from documents that exist — most live at
   ...
   are **living documents** shared across every feature in this app, at
   `.specify/service/{doc}.md` — see "Living Documents & Cross-Feature
   Impact" below. `api-spec.md` is NOT a living document in this pack —
   this pack only consumes an API, so its contract stays per-feature in
   `design.md` §3.
...
`security-design.md` and `data-model.md` (Local Data & Cache Model) are
**living documents** — shared across every feature in this app, at
`.specify/service/{doc}.md`, not one copy per feature. `api-spec.md`
stays per-feature, written into `design.md` §3 (this pack only consumes
an API). Every other document in the chain above is per-feature, as
shown.
...
key-value entry, a threat entry) that a **different** feature also
depends on — and that other feature's own `srd.md`/`design.md` are never
read during a normal `/change` walk, since the walk is scoped to the
feature that raised the CR.
...
at `.specify/service/` — screen-spec, ux-flow, and LLD).
...
| New screen/flow | use-cases + srd + design + lld |
| New actor or use case | use-cases + srd |
| New local table/entity or storage key | srd + data-model (living — check cross-feature impact) |
...
# 2. Run /change
/change "scope upgraded from pilot to mvp — need data-model, screen-spec, ux-flow, LLD"

# 3. /change will:
#    - ANNOTATE upstream docs (context, brd, use-cases, srd, security §1)
#    - INCORPORATE data-model, screen-spec, ux-flow if this is the first
