...
class TestFillMode:
    ...
    def test_all_cli_flags_supplied_only_ai_tool_prompted(self, runner, tmp_path, monkeypatch):
    ...
    def test_context_file_not_overwritten_if_it_already_exists(self, runner, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
    ...
    def test_micro_manifest_skips_scope_and_type_prompts(self, runner, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        manifest_dir = tmp_path / ".specify"
    ...
    def test_invalid_project_name_raises(self, runner, tmp_path, monkeypatch):
        ...
        with patch("questionary.select", return_value=_Answer("claude-code")):
            result = runner.invoke(init_command, [
                "-p", 'bad"name', "-f", "checkout", "-s", "mvp", "-t", "backend-service",
            ])
...
class TestScaffoldMode:
    ...
    def test_scaffolds_recommended_pack_and_fills_manifest(self, runner, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        ...
        with patch("sdd.commands.init.detect_project_type", return_value="backend-service"), \
             patch("sdd.commands.init.scaffold_pack", side_effect=_fake_scaffold_pack) as scaffold, \
             patch("questionary.select", side_effect=[
                 _Answer("sdd-backend-service"), _Answer("claude-code"),
             ]), \
