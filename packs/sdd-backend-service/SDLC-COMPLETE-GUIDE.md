...
| 5 | `/specify-doc {name}` | Extended docs: security, data-model (both living, `.specify/service/`), resilience… | SRD approved |
...
| `/specify-doc {name}` | security / data-model / resilience / investigation — `security` and `data-model` are **living, service-level** documents at `.specify/service/`, extended by every feature after the first (SKIP/ADD-unit/UPDATE-unit), never regenerated | SRD approved |
...
+ Service NFR Baseline (Performance/Availability/Throughput/Data Retention — filled once
by the first feature to reach `/specify-srd`, referenced by every later feature instead
of restating the same numbers) + Core Principles, Domain Rules, Never Do

**Spec document inventory by scope:**
```
pilot: brd → use-cases → srd → security-design (§1)
mvp:   + data-model → security-design (§1-2)   [api-spec is extracted later, at /plan-design §3]
...
3. Deployment Plan — the strategy and rollback steps are standard for this
   service, not re-derived per release: pulled from
...
4. Post-Deploy Smoke Test — the checks themselves are standard, pulled
   from the runbook; only this release's specific endpoint/NFR target is
   filled in
...
- [ ] Additional `/specify-doc` calls for data-model (mvp+, living), resilience (full)
