...
MIGRATIONS: list[Migration] = [
    ...
    },
    {
        ...
        "to": "2.8.35",
        "description": (
            "Full review-gate audit across every living and feature "
            "document: closed two behavioural gaps (validate/analyze/"
            "clarify missing the Confluence-only submission fallback; "
            "api-spec.md's first version never getting a review "
            "ticket/page) and fixed a sync-tooling bug that was silently "
            "serving 10 pack prompts a stale copy of the approval logic"
        ),
        "notes": [
            "Requested audit: 'perform an audit to see all living "
            "document and feature document create the review ticket and "
            "pushed to confluence page and review is pulled from jira "
            "and confluence to make sure all are having the same "
            "process if jira or confluence is active, else it can be "
            "done in chat'",
            "Finding A -- validate.prompt.md, analyze.prompt.md, "
            "clarify.prompt.md (hand-authored, not shared, identical "
            "across all 5 packs) only ever called `sdd review submit` "
            "and fell straight to pure chat mode on failure. review.py's "
            "review_submit requires BOTH jira: and confluence: sections "
            "and hard-fails otherwise (confirmed by reading the CLI) -- "
            "so in a confluence-only project (no jira:) these three "
            "documents never reached Confluence, unlike every other "
            "reviewed document, which falls through to `sdd confluence "
            "draft` first. Fixed by swapping their Step B/C onto the "
            "same submit-for-review-step/review-decision-step shared "
            "blocks the other 12 commands already use, adding the "
            "markers to all 5 packs' copies of the 3 files and running "
            "sync-blocks.sh -- each file's own document-specific "
            "approval-scope caveat (e.g. validate.md's per-item "
            "checkboxes not being itemized by a blanket approval) was "
            "preserved as trailing prose after the shared block, not "
            "lost in the swap",
            "Finding B -- api-spec.md (the third living document, "
            "generated inside plan-design.prompt.md when a service-"
            "providing feature is planned) had no submission step on "
            "first creation, unlike its sibling living docs (data-model, "
            "security-design), which get one via specify-doc.prompt.md's "
            "shared block. api-spec.md only ever reached Confluence/Jira "
            "later, when a subsequent feature called `sdd review apply "
            "--doc api-spec` on an update -- meaning a service's first "
            "feature could ship an API contract that was never actually "
            "routed through a review ticket. Fixed by adding the same "
            "shared submission block (doc_key = api-spec) right after "
            "first-time generation, independent of design.md's own "
            "later approval step",
            "Finding C (found while fixing B, not part of the original "
            "two) -- sync-blocks.sh's two loops ran in the wrong order: "
            "the blocks loop (which refreshes shared:{id} marker regions "
            "in pack files) ran BEFORE the full-file loop (which copies "
            "_shared/full/{path} whole-file into every pack). Ten "
            ".github/prompts/ files in full/ (specify-brd, specify-uc, "
            "specify-srd, specify-doc, plan-arch, plan-hld, plan-adr, "
            "plan-design, plan-lld, task) themselves embed "
            "submit-for-review-step/review-decision-step markers -- "
            "full/ is never itself touched by the blocks loop (it only "
            "globs ../sdd-*/), so those 10 files' own inline copies had "
            "silently drifted stale (missing review-decision-step's "
            "'resolve the approver's name from roles.yml, fill the "
            "Approver column' step, added to blocks/review-decision-"
            "step.md at some point after these 10 files were last hand-"
            "synced). Every sync-blocks.sh run: blocks loop correctly "
            "refreshed the 10 files in all 5 packs, then the full-file "
            "loop immediately overwrote them back to the stale full/ "
            "copy, since cmp always showed a diff. Verified directly: "
            "validate.prompt.md (never full-file-synced) had the fresh "
            "content, specify-brd.prompt.md (full-file-synced) did not, "
            "in the same pack, same run. Fixed two ways: (1) swapped the "
            "loop order in sync-blocks.sh so the blocks pass always runs "
            "last and has the final word on every marker region "
            "regardless of which loop last touched the file; (2) "
            "refreshed the 10 full/ files' own embedded copies to match "
            "blocks/*.md exactly, so a maintainer reading full/ directly "
            "isn't misled either. Confirmed idempotent: three consecutive "
            "sync-blocks.sh runs after the fix produce zero further file "
            "changes. Added a README.md rule documenting the gotcha so "
            ...
            "three consecutive sync-blocks.sh runs confirmed idempotent "
    ...
    },
    {
        "from": "2.9.21",
        "to": "2.9.22",
        "description": "validate.prompt.md's CHECKLIST GATE now actually "
        "blocks at mvp/full scope, matching what CLAUDE.md already "
        "claimed -- found via a live dashboard report ('Next: Run "
        "/checklist' shown even though Validate/Analyze/Clarify/Design/"
        "LLD/Stories were all already approved)",
        "notes": [
            "Root cause: CLAUDE.md's Scope Reference table and the "
            "/checklist section both document /checklist as 'Mandatory "
            "for mvp and full scope ... All CRITICAL items must be "
            "resolved before /validate can proceed' -- but every pack's "
            "validate.prompt.md Step 0 ('CHECKLIST GATE (advisory)') was "
            "hard-coded to never block, regardless of scope ('Do NOT "
            "block validate -- /checklist is optional'). A project could "
            "sail from SRD straight through /validate/-analyze/-clarify/"
            "-design/-plan-lld/-task, all approved, without /checklist "
            "ever running -- exactly contradicting the pack's own "
            "documented policy. The dashboard's 'Next: Run /checklist' "
            "was accurate the whole time; the framework just never "
            "enforced what it claimed",
            "Fixed in all 5 packs' validate.prompt.md Step 0: now "
            "scope-aware -- pilot stays advisory (warn, don't block), "
            "mvp/full now genuinely BLOCK when checklists/ is missing "
            "(never run) or has open CRITICAL items. sdd-micro excluded "
            "(no /checklist in that pack by design)",
            "Fixed all 5 packs' CLAUDE.md: the self-contradictory "
            "section heading '/checklist -- Optional Spec-Quality Gate' "
            "(directly contradicted the next line, 'Mandatory for mvp "
            "and full scope') is now just '/checklist -- Spec-Quality "
            "Gate'",
            "Real behavior change for existing mvp/full-scope projects: "
            "a project that previously skipped /checklist and already "
            "has Validate approved will now be BLOCKED on its next "
            "/validate run until /checklist is run and CRITICAL items "
            "resolved -- catching up late is expected and intended, not "
            "a regression",
            "This Node CLI ships from the same pack sources -- this "
            "migration entry exists so both CLIs report the same "
            "sdd_version chain",
            ...
            "check-cross-references.py clean across all 6 packs; "
            "sync-blocks.sh run twice consecutively with zero "
    ...
    },
    {
        "from": "2.9.23",
        "to": "2.9.24",
        "description": "sdd dashboard: Tasks pagination, Documents/BO "
        "toggles extended to per-feature cards, and a real bug fix -- "
        "none of v2.9.23's collapsible sections actually survived the "
        "5s poll -- found via direct user follow-up asking for a full "
        "UX review of the dashboard",
        "notes": [
            "Client-side only (dashboard_static/app.js + style.css), "
            "plus one test updated in test_dashboard.py for "
            "renderJiraExport()'s new third parameter -- no other "
            "Python/server-side changes",
            "Tasks pagination: a real project's tasks.md can run to "
            "50-200+ entries -- renderTasks() now paginates at 20 "
            "rows/page (TASKS_PAGE_SIZE) with Prev/Next buttons and a "
            "'Showing X-Y of N - page P/C' indicator, state.taskPage "
            "kept per-feature so switching tabs preserves position. "
            "The aggregate progress bar/summary line above the table "
            "always reflects the FULL list, not just the current page",
            "Documents (per-feature) and Business Objectives "
            "(per-feature) are now collapsible like the project-wide "
            "sections already were -- Documents defaults open (bounded "
            "list), Business Objectives defaults open only at <=8 BOs "
            "(BO_OVERVIEW_AUTO_COLLAPSE_THRESHOLD, also retrofitted "
            "onto the existing project-wide BO Overview, which "
            "previously always defaulted open regardless of size)",
            "Jira Export's Stories/Tasks lists (comma-joined ticket "
            "keys, not a table, but still a wall of ~100 links on a "
            "big feature) truncate at 12 items with a '+N more'/'show "
            "less' toggle (JIRA_EXPORT_LIST_LIMIT)",
            ...
            "rebuilt wholesale on every 5s poll, so a manual toggle "
            "silently snapped back to its hardcoded default every 5 "
            "seconds. Fixed with state.collapsed (keyed by a stable "
            "data-section-id per <details>) plus a capture-phase "
            "'toggle' listener on #root -- native 'toggle' events don't "
            "bubble, so capture phase is required for delegation to "
            "reach them at all -- and a new sectionOpenAttr(id, "
            "defaultOpen) helper that only falls back to the smart "
            "default when the user hasn't chosen explicitly yet. "
            "Applied to all four collapsible sections",
            "This Node CLI ships from the same pack sources -- this "
            "migration entry exists so both CLIs report the same "
            "sdd_version chain (no dashboard of its own, unaffected "
            "beyond the version stamp)",
            "Verified: cli-python pytest 993/993 (992 unchanged + 1 "
            "fixed for renderJiraExport's new signature); ruff "
            "check/format clean; mypy clean; node --check app.js valid "
            "syntax. Manually verified end-to-end with a real HTTP "
            "server against a synthetic 87-task/14-BO/87-Jira-key "
            "fixture, screenshotted via Playwright/Chromium: pagination "
            "page 1/5 -> 2/5 transitions correctly, the 14-BO section "
            "auto-collapsed as designed, Jira Export's '+75 more' "
            "toggle works, and the persistence fix was directly "
            "verified by manually opening a collapsed section then "
            "calling the app's own refresh() (simulating the 5s poll) "
    ...
    },
    {
        "from": "3.0.0",
        "to": "3.0.1",
        "description": "Strengthened the shared Token Usage Logging "
        "step (synced to 19 prompt files x 5 packs) after a live user "
        "report during /implement: logging was silently skipped for "
        "every task once the user asked to proceed through all tasks "
        "without stopping for 'go' between them",
        "notes": [
            "User's scenario: told the agent to proceed through every "
            "/implement task without stopping for per-task 'go'. The "
            "agent's own transcript showed it interpreting this "
            "broadly -- it also silently skipped the Token Usage "
            "Logging step for every task, even though "
            "token-pricing.yml was already configured and working "
            "(confirmed: other commands in the same project -- "
            "specify-brd, validate, etc. -- had logged fine)",
            "Root cause: the logging step sits immediately adjacent to "
            "the 'WAIT for go' instruction in every prompt that has "
            "it, with nothing telling the agent the two are "
            "independent -- an agent optimizing for 'proceed without "
            "stopping' could plausibly read that as license to also "
            "skip the adjacent housekeeping step, since nothing said "
            "otherwise",
            "Fixed: added a paragraph to "
            "packs/_shared/blocks/token-usage-log-step.md clarifying "
            "that 'proceed without stopping' / 'skip confirmation' "
            "instructions waive the PAUSE between steps, not the "
            "logging step -- it must still run after every single "
            "task/command execution regardless, even mid-way through "
            "a batch, since skipping it silently produces an "
            "incomplete token-usage.md that under-reports cost",
            "Pack-content-only (prompt instruction text for the AI "
            "agent) -- no CLI code touched",
            "This Node CLI ships from the same pack sources -- this "
            "migration entry exists so both CLIs report the same "
            "sdd_version chain",
            "Verified: check-cross-references.py clean across all 6 "
            "packs; sync-blocks.sh run three times consecutively with "
            "zero drift after the first sync (96 files touched: 95 "
            "synced pack prompts + 1 canonical block source); "
            "test-setup.sh 19/19 passed; test-setup-micro.sh 12/12 "
    ...
    },
    {
        "from": "3.0.2",
        "to": "3.1.0",
        "description": "Two purely-additive discoverability links -- "
        "`sdd --help` and the dashboard now both point at where to "
        "report a bug or request a feature",
        "notes": [
            "Follow-up to setting up GitHub Issues (templates, "
            "CONTRIBUTING.md, SECURITY.md, making the repo public) "
            "earlier the same session -- those solved 'does a place to "
            "report issues exist', this solves 'does anyone using the "
            "CLI or dashboard actually see it', since most sddflow "
            "users interact via pip install + CLI/dashboard and may "
            "never visit the GitHub repo page directly",
            "cli-python/sdd/__main__.py: the top-level `sdd` Click "
            "group gained an epilog shown at the bottom of `sdd "
            "--help` -- 'Found a bug or have a feature request? "
            ...
            "Also added (not part of this version bump -- prose/"
            "governance files sdd upgrade never reads, same bucket as "
            "CONTRIBUTING.md/SECURITY.md): CODE_OF_CONDUCT.md "
            "(Contributor Covenant v2.1) and "
            ".github/PULL_REQUEST_TEMPLATE.md, closing the same gap "
            "as GitHub's spec-kit repo's community-health sidebar",
            "This Node CLI ships from the same pack sources -- this "
            "migration entry exists so both CLIs report the same "
            "sdd_version chain (the Node CLI has no dashboard or "
            "equivalent --help epilog of its own -- scaffolding-only "
            "by design -- so nothing here actually applies to it "
            "beyond the version stamp)",
            "Verified: cli-python pytest 1013/1013 (unchanged -- no "
            "test asserts --help/footer output verbatim); ruff "
            "check/format clean; mypy clean (35 source files); `sdd "
            "--help` manually confirmed to show the new epilog",
        ],
    },
    {
        "from": "3.1.0",
        "to": "3.1.1",
        "description": "URL correction following the GitHub repo rename "
        "(sunil1983us/universalguide -> sunil1983us/sddflow) -- pure "
        "bug fix, no new behavior",
        "notes": [
            "The repo was renamed to match the actual product branding "
            "(sddflow is already the PyPI package name, npm package "
            "name, and CLI binary name -- universalguide never matched "
            "any of them). GitHub's redirect covers old links "
            "indefinitely, but v3.1.0's --help epilog and dashboard "
            "footer link should say what's actually true rather than "
            "rely on a redirect forever",
            "cli-python/sdd/__main__.py's --help epilog and "
            "cli-python/sdd/commands/dashboard_static/page.html's "
            "footer link both updated to "
            "https://github.com/sunil1983us/sddflow/issues",
            "Also corrected in the same sweep (no bump needed on their "
            "own -- prose/governance files sdd upgrade never reads): "
            "README.md, CONTRIBUTING.md, SECURITY.md, CHANGELOG.md's "
            "newest entry, pyproject.toml/package.json URLs, the issue "
            "templates, and each pack's README/QUICKSTART/"
            "FLOW-ROLES-GATES.md",
            ...
            "(unrelated to the repo name, coincidental match) and an "
            "old CHANGELOG.md entry documenting a past release's git "
            "clone example (historical record, not rewritten)",
            "This Node CLI ships from the same pack sources -- this "
            "migration entry exists so both CLIs report the same "
            "sdd_version chain (the Node CLI has no dashboard or "
            "--help epilog of its own -- scaffolding-only by design -- "
            "so nothing here actually applies to it beyond the version "
            "stamp)",
            "Verified: cli-python pytest 1013/1013; ruff check/format "
            "clean; mypy clean (35 source files); bandit 0 issues; "
            "node 28/28; grep sweep confirms zero remaining "
            "'universalguide' references outside the two intentionally "
            "-excluded files",
        ],
    },
    {
        "from": "3.1.1",
        "to": "3.2.0",
        "description": "New `sdd doctor` command -- read-only report of "
        "drift between a project's framework-managed files and the "
        "currently installed CLI's pack content",
        "notes": [
            "First step of a larger effort responding to external review "
            "feedback (verified independently before acting on any of "
            "it -- radon complexity numbers and coverage percentages "
            "matched exactly when re-run): sdd upgrade has only ever "
            "stamped manifest.yml's sdd_version field, never actually "
            "syncing a project's templates/prompts/commands/"
            "instructions/setup-scripts/workflows/.cursor/.vscode files "
            "forward to match the installed CLI's pack content -- so a "
            "project could report itself 'upgraded' while every one of "
            "those files still matched whatever version it was "
            "originally scaffolded with",
            "sdd doctor is deliberately read-only -- reports drift, "
            "changes nothing -- so it's safe to ship and use "
            "immediately, ahead of the much bigger sdd upgrade rewrite "
            "that will actually apply fixes (a later bump)",
            "New cli-python/sdd/utils/managed_files.py: SHA-256-hashes "
            "every managed file in the pack bundled with the currently "
            "installed CLI (the 'canonical' content) and in the "
            "project itself, classifying each as up-to-date / missing "
            "/ needs-update / user-modified / differs-for-unknown-"
            "reason (the last one being the honest fallback for every "
            "project scaffolded before this existed -- no baseline "
            "recorded, so it can't distinguish a pack update from a "
            "hand edit, and doesn't guess)",
            ...
            "silently guess the wrong pack for any sdd-universal-"
            "scaffolded project, since sdd-universal handles every "
            "project_type from one shared set of files rather than "
            "becoming a type-specific pack, and manifest.yml has never "
            "recorded an explicit 'pack:' field to disambiguate. "
            "Root-caused but not fixed here -- doctor detects when "
            "_resolve_pack's answer came from inference rather than an "
            "explicit stored field and prints a prominent warning "
            "instead of silently presenting a guess as fact; a real "
            "fix belongs with the pack_version/manifest-schema split "
            "planned next in this phased effort",
            "This Node CLI ships from the same pack sources -- this "
            "migration entry exists so both CLIs report the same "
            "sdd_version chain (the Node CLI has no doctor command of "
            "its own -- scaffolding-only by design -- so nothing here "
            "actually applies to it beyond the version stamp)",
            "Verified: cli-python pytest 1035/1035 (1012 unchanged + 23 "
            "new covering inventory building, hashing, baseline "
            "loading/malformed-JSON handling, all 5 classification "
            "branches directly, and the CLI wrapper's exit codes/"
            "--quiet/inference-warning banner); manually verified "
            "end-to-end against a real freshly-scaffolded project (113/"
            "113 up to date with the correct pack; a hand-edit and a "
            "deletion both caught precisely, nothing else flagged); "
            "ruff check/format clean; mypy clean (37 source files, up "
            "from 35); bandit 0 issues",
        ],
    },
    {
        "from": "3.2.0",
        "to": "3.3.0",
        "description": "New required-going-forward `pack:` field in "
        "manifest.yml, closing the pack-identity gap sdd doctor's "
        "warning banner (added in 3.2.0) could only detect, not fix",
        "notes": [
            "Direct follow-up to 3.2.0: that release shipped sdd doctor "
            "with a warning banner for the case where _resolve_pack() "
            "had to *infer* a project's pack from project_type rather "
            "than read it from an explicit field -- most visibly wrong "
            "for sdd-universal, which serves all 10 project_types from "
            "one shared set of files and was being silently resolved "
            "to a type-specific pack (e.g. sdd-backend-service) instead "
            "of itself",
            "This bump fixes it at the source instead of just flagging "
            "it: every pack's own manifest.yml template "
            "(sdd-backend-service, sdd-frontend-spa, sdd-fullstack, "
            "sdd-mobile, sdd-universal, and sdd-micro outside the "
            'lockstep) now bakes in a static `pack: "sdd-..."` line '
            "right after sdd_version, the same pattern already used for "
            "sdd_version's own static default -- setup.sh/setup.ps1 "
            "need no template-substitution logic, the value is simply "
            "correct for every project scaffolded from that pack",
            "_resolve_pack() in upgrade.py already prioritized "
            "manifest.get('pack') above project_type inference (added "
            "in the 3.2.0 work) -- this bump is what actually makes "
            "that branch fire for setup.sh/setup.ps1-scaffolded "
            "projects; sdd init-scaffolded projects were already "
            "unaffected, since init has stamped manifest_patch['pack'] "
            "since before this phased effort started",
            "PACK-SPEC.md's documented manifest.yml schema updated to "
            "list `pack:` as a required field alongside sdd_version, "
            "so community pack authors building against PACK-SPEC.md "
            "include it from day one",
            "Existing projects on older sdd_version have no `pack:` "
            "field yet -- this migration entry only stamps sdd_version "
            "forward, as always; a project without the field simply "
            "keeps falling back to inference (with the 3.2.0 warning "
            "banner) until it's re-scaffolded or the field is added by "
            "hand -- there is no destructive or surprising behavior "
            "change for anyone upgrading",
            "This Node CLI ships from the same pack sources -- this "
            "migration entry exists so both CLIs report the same "
            "sdd_version chain (the Node CLI has no pack-resolution "
            "logic of its own -- scaffolding-only by design -- so "
            "nothing here actually applies to it beyond the version "
            "stamp)",
            "Verified: cli-python pytest 1035/1035 (unaffected by the "
            "manifest-only change); all 6 packs' manifest.yml confirmed "
            "to parse as valid YAML with the new field; "
            "check-cross-references.py clean across all 6 packs; "
            "test-setup.sh 19/19 and test-setup-micro.sh 12/12 passed; "
            "two real functional scaffolds re-confirmed the fix end-to-"
            "end -- a fresh sdd-backend-service scaffold now resolves "
            "via \"manifest.yml 'pack' field\" with no warning, and a "
            "fresh sdd-universal scaffold (setup.sh --type "
    ...
    },
    {
        "from": "3.4.0",
        "to": "3.4.1",
        "description": "Bugfix: `sdd config test` no longer crashes with a "
        "raw traceback when a Jira/Confluence server responds 200 with a "
        "non-JSON body",
        "notes": [
            "Real user report: testing PAT auth against two Jira/"
            "Confluence Data Center servers, `sdd config test` crashed "
            "with an unhandled `json.decoder.JSONDecodeError: Expecting "
            "value: line 1 column 1 (char 0)` traceback instead of a "
            "clean per-service status line",
            "Root cause: response.json() raises this when the server "
            "answers 200 with a non-JSON body -- most commonly an SSO/"
            "login-page redirect, or base_url pointing at the wrong "
            "path entirely. `except requests.HTTPError` never caught "
            "it, since JSONDecodeError is a RequestException *sibling*, "
            "not a subclass of HTTPError -- an easy exception-hierarchy "
            "mistake to make and never notice until it happens for real",
            "Fixed by extracting the duplicated Jira/Confluence probe "
            "logic in sdd/commands/config.py into one _probe_service() "
            "helper with three distinct except clauses: HTTPError (the "
            "server said no, show its status+body), JSONDecodeError "
            "(caught both as the bare stdlib class -- older requests "
            "versions raise that directly, matching what the reporting "
            "user's traceback showed -- and requests.exceptions."
            "InvalidJSONError, the modern requests>=2.27 wrapper -- "
            "shows an actionable message pointing at base_url/SSO/token "
            "validity instead of a raw parse error), and any other "
            "RequestException (connection refused, DNS failure, "
            "timeout, TLS error, malformed URL -- requests' own message "
            "is clear enough on its own)",
            "No manifest.yml or generated-file changes -- purely a "
            "crash fix in CLI error handling, nothing for an existing "
            "project to migrate",
            "This Node CLI ships from the same pack sources -- this "
            "migration entry exists so both CLIs report the same "
            "sdd_version chain (the Node CLI has no `sdd config test` "
            "of its own -- scaffolding-only by design -- so nothing "
            "here actually applies to it beyond the version stamp)",
            "Verified: cli-python pytest 1098/1098 (1094 unchanged + 4 "
            "new covering all three failure modes, including the exact "
            "bare-stdlib-JSONDecodeError reproduction of the reported "
            "bug and the modern requests-wrapped variant); ruff check/"
            "format clean; mypy clean (37 source files); bandit 0 "
            "issues; manually confirmed end-to-end through the full "
            "Click CLI invocation path (not just the unit tests) -- a "
            "probe against a real unreachable server produces a clean "
    ...
    },
    {
        "from": "3.5.0",
        "to": "3.6.0",
        "description": "/clarify now interviews live chat answers one item "
        "at a time and pushes back on vague ones, instead of posting the "
        "whole list and passively waiting",
        "notes": [
            ...
            "about items one at a time -- CRITICAL/HIGH first -- and, if "
            "an answer is still vague, asks one specific follow-up "
            "instead of accepting it. Pushes back at most twice per item; "
            "a third vague answer resolves by best guess instead of "
            "looping forever. A human who pastes answers for several "
            "items at once is still accepted immediately -- the "
            "one-at-a-time cadence is the default, not a hard rule",
            "The three other intake paths are unchanged: editing "
            "clarify.md directly and saying 'done', 'best guess'/"
            "'continue', and an async Jira/Confluence comment reply "
            "(pulled in by `sdd review pull-answers --doc clarify`) all "
            "still bypass the interview entirely -- only the live-chat "
            "answer path changed",
            "Prompt-content only (packs/{sdd-backend-service,sdd-"
            "frontend-spa,sdd-fullstack,sdd-mobile,sdd-universal}/"
            ".github/prompts/clarify.prompt.md) -- no CLI code touched, "
            "no manifest.yml schema change. sdd-micro is unaffected (no "
            "/clarify in its 3-command scope)",
            "This Node CLI ships from the same pack sources -- this "
            "migration entry exists so both CLIs report the same "
            ...
            "unexpected drift (only the 5 intentionally-edited "
            "clarify.prompt.md files changed); check-cross-references.py "
            "clean across all 6 packs; test-setup.sh 19/19 passed",
        ],
    },
    {
        "from": "3.6.0",
        "to": "3.6.1",
        "description": "status.py's _local_jira_links() now traversal-checks "
        "the feature name before building docs/jira/{feature}/keys.yml, "
        "closing a CodeQL-flagged path-injection gap",
        "notes": [
            "_local_jira_links(root, feature, base_url) built "
            "docs/jira/{feature}/keys.yml by string concatenation -- "
            "root / 'docs' / 'jira' / feature / 'keys.yml' -- with no "
            "traversal check of its own. Every current caller already "
            "validated `feature` before calling in (dashboard.py's "
            "/api/review-links endpoint via the _SAFE_TOKEN allowlist "
            "regex; the CLI sdd status path, same trust level as a "
            "user's own manifest.yml), so this was never actually "
            "reachable with an attacker-controlled value -- but CodeQL's "
            "py/path-injection query correctly flags that the function "
            "itself provides no guarantee, only its callers happen to",
            ...
            "tasks: []} result as 'no keys file found' instead of "
            "reading whatever's at the escaped path",
            "Same shape and precedent as the 3.4.2 -> 3.4.3 git_host.py "
            "CodeQL fix: narrow practical exploitability given the "
            "existing caller-side guards, but the fix is free and the "
            "old behavior was genuinely under-guarded regardless",
            "This Node CLI ships from the same pack sources -- this "
            "migration entry exists so both CLIs report the same "
            "sdd_version chain (the Node CLI has no status/Jira-links "
            "logic of its own -- scaffolding-only by design -- so "
            "nothing here actually applies to it beyond the version "
            "stamp)",
            "Verified: cli-python pytest 1144/1144 (1143 unchanged + 1 "
            "new -- test_jira_keys_rejects_a_traversal_feature_name, "
            "confirming a file placed outside docs/jira/ is never read "
            "for a '../../outside'-style feature value); ruff "
            "check/format clean; mypy clean (38 source files); bandit 0 "
            "issues",
        ],
    },
    {
        "from": "3.6.1",
        "to": "3.7.0",
        "description": "/clarify's live interview redesigned again -- "
        "spec-kit-inspired format: pre-reasoned recommended answers, "
        "at least 5-6 questions balanced across item types",
        "notes": [
            "3.6.0 made /clarify's live-chat path push back on a vague "
            "answer with one narrower follow-up (max twice) before "
            "falling to best guess. Checked github/spec-kit's own "
            "clarify.md template directly for a second opinion on this "
            "-- it asks one question at a time (capped at 5), each with "
            "a pre-reasoned recommended/suggested default the human can "
            "accept with 'yes' instead of typing an answer, drawn from a "
            "9-category taxonomy that inherently balances technical and "
            "business angles",
            "Adopted that shape instead: every question now states "
            "**Recommended: Option X** (multiple-choice, lettered table) "
            "or **Suggested:** (open-ended), reasoned from something "
            "real in this project's own docs (context.md, "
            "constitution.md's Tech Stack/Domain Rules, brd.md's stated "
            "business objectives) -- never a generic justification. "
            "Ask through at least 5-6 items when that many are open, "
            "picked across item types (not just severity) so it doesn't "
            "ask five near-identical AMB items back to back while a "
            "real GAP/OQ item sits unasked",
            "The push-back mechanic is gone -- with a recommended "
            "default always on offer, 'make it reasonable'-style "
            "non-answers are rarer to begin with. What remains: a reply "
            "that doesn't engage at all gets exactly one check-in "
            "referencing the already-offered default (not a new/"
            "repeated question); if that doesn't land, the item "
            "resolves using that same default as agent-best-guess "
            "rather than looping",
            ...
            "frontend-spa,sdd-fullstack,sdd-mobile,sdd-universal}/"
            ".github/prompts/clarify.prompt.md) -- no CLI code touched, "
            "no manifest.yml schema change. sdd-micro unaffected",
            "This Node CLI ships from the same pack sources -- this "
            "migration entry exists so both CLIs report the same "
            "sdd_version chain (the Node CLI has no prompt-execution "
            "role of its own -- scaffolding-only by design -- so "
            "nothing here actually applies to it beyond the version "
            "stamp)",
            "Verified: sync-blocks.sh run twice consecutively with zero "
            "unexpected drift (only the 5 intentionally-edited "
    ...
    },
    {
        "from": "3.7.0",
        "to": "3.7.1",
        "description": "Security/correctness: fixed unencoded file I/O "
        "across the whole cli-python codebase -- non-ASCII content "
        "(em-dashes, curly quotes, accents) got mangled on Windows and "
        "other non-UTF-8-locale systems",
        "notes": [
            "A user hit this directly: an em-dash in a /create-context-"
            "generated context.md came out as mojibake ('â€”') "
            "once pushed to Confluence via `sdd confluence push`. Root "
            "cause: Path.read_text()/write_text() and open()/"
            "os.fdopen() in text mode all default to "
            "locale.getpreferredencoding(False) when encoding= is "
            "omitted -- cp1252 on many Windows setups, not UTF-8. Each "
            "UTF-8 byte of the em-dash got reinterpreted as a separate "
            "cp1252 character",
            "Root-caused to confluence.py's md_path.read_text() first, "
            "then found the identical bug in the shared "
            "atomic_write_text() utility (os.fdopen(fd, 'w'), no "
            "encoding) used by nearly every write site in the codebase "
            "(manifest.py, review.py x6, jira.py, hooks.py, "
            "confluence_push_log.py, managed_files.py), plus ~19 more "
            "read_text/write_text call sites across commands/config.py, "
            "commands/confluence.py, commands/cr.py, commands/"
            "dashboard.py, commands/hooks.py, commands/init.py, "
            "commands/jira.py, commands/pr.py, commands/review.py, "
            "commands/token_log.py, utils/atlassian_auth.py, utils/"
            "claude_code_transcript.py, utils/detect.py, utils/"
            "integrations.py, utils/sdd_parser.py, utils/status.py, "
            "utils/dashboard_comments.py",
            "Fixed all of them: every read_text()/write_text() call and "
            "the one os.fdopen() call now pass encoding='utf-8' "
            "explicitly, no longer falling back to OS locale. One "
            "f-string quote collision found along the way (pr.py -- "
            "can't reuse double quotes inside a double-quoted f-string "
            "pre-3.12) fixed by switching that one site to single quotes",
            ...
            "same sdd_version chain -- nothing in the Node CLI needed "
            "to change",
            "Added cli-python/tests/test_utf8_encoding_everywhere.py: a "
            "static AST scan across every file under sdd/ asserting "
            "every text-mode read_text()/write_text()/open()/fdopen() "
            "call specifies encoding= (binary-mode opens correctly "
            "excluded) -- this is the actual regression guard, "
            "confirmed to fail when encoding= is missing regardless of "
            "the test runner's own locale",
            "Verified: cli-python pytest 1146/1146 (1144 unchanged + 2 "
            "new); ruff check/format clean; mypy clean (38 source "
    ...
    },
    {
        ...
        "notes": [
            ...
            "/rest/api/{version}/search/jql. That endpoint replaced "
            "Cloud's deprecated GET /search (410 Gone) but was never "
            "added to Server/Data Center, which still only has the "
            "classic /rest/api/2/search. deployment='server' correctly "
            "switched the API version prefix (2 vs 3) but search() never "
            "branched the path itself, so every Server/DC search 404'd",
            "search() now posts to /search/jql only when "
            "_api_version == '3' (Cloud); Server/DC (_api_version == '2') "
            "posts to plain /search instead -- same request/response "
            "shape, just the endpoint Server/DC actually has",
            "Also clarifies, for the 'is the epic's project read from "
            "integrations.yml' question that prompted digging into this: "
            "yes -- JiraConfig.key_for('feature') resolves the Jira "
            "project key from integrations.yml's jira.project_key (or a "
            "project_keys.feature/epic override) every push; the actual "
            "Epic issue is found fresh each time via a live JQL label "
            "search (find_by_label, sdd-feature:{feature}), never a "
            "cached key file -- so a stale local key can't be the cause "
            "of a 404 here, only the wrong endpoint could",
            ...
            "Verified: cli-python pytest 1151/1151 (1149 unchanged + 2 "
            "new -- confirmed the new Server/DC test fails against the "
            "pre-fix code with the exact 404-causing URL and passes "
    ...
    },
    {
        ...
        "notes": [
            ...
            'in position 15: invalid start byte" inside read_manifest(). '
            "Root cause: before v3.7.1, write_manifest()/atomic_write_text() "
            "had no explicit encoding=, so on a non-UTF-8 system locale "
            "(cp1252 is the default on most Windows installs) any "
            "non-ASCII character -- including this file's own header "
            "em-dash -- got written as cp1252's single byte (0x97) "
            "instead of UTF-8's three bytes (E2 80 94). v3.7.1 made "
            "read_manifest() strictly require UTF-8, which is correct "
            "for files written by 3.7.1+, but any manifest.yml written "
            "before that fix, on a cp1252-locale system, and never "
            "rewritten since, now fails to decode at all -- a hard "
            "crash blocking every command, not just upgrade",
            ...
            "overwhelmingly likely culprit given our own write-side "
            "history) -- and immediately rewrites the file as real UTF-8 "
            "via write_manifest() so every other command reading the "
            "same file self-heals too, not just this one call. If "
            "neither UTF-8 nor cp1252 can decode it, raises ManifestError "
            "with a clear, actionable message instead of a raw traceback "
            "-- same pattern the existing corrupt-YAML branch already "
            "used, extended to cover corrupt-encoding too",
            "This Node CLI has no manifest.yml encoding logic of its own "
            "(scaffolding-only by design, and its own write path already "
            "defaults to UTF-8 in Node) and is unaffected by this fix "
            "beyond the version stamp -- this migration entry exists so "
            "both CLIs report the same sdd_version chain",
            "Verified: cli-python pytest 1153/1153 (1151 unchanged + 2 "
            "new -- confirmed both new tests fail against the pre-fix "
            "code, reproducing the exact reported UnicodeDecodeError, "
    ...
    },
    {
        ...
        "notes": [
            ...
            "nor the AI agent driving the CLI on their behalf could say "
            "more than the status code -- because nothing in this "
            "codebase ever looked at the response body. requests."
            "HTTPError's default str() is just '400 Client Error: Bad "
            "Request for url: ...'; Jira/Confluence both put the actual "
            "reason (e.g. Jira's errors/errorMessages JSON -- in this "
            "case, a required custom field) in the body, which every "
            "single raise_for_status() call in jira_client.py (11 call "
            "sites) and confluence_client.py (9 call sites) silently "
            "discarded",
            "Added sdd/utils/http_errors.py: raise_for_status_with_body(), "
            "a shared drop-in replacement for r.raise_for_status() that "
            "folds r.text into the raised exception's message while "
            "preserving .response (chained via `from e`) so callers that "
            "branch on status code -- e.g. ConfluenceClient.upsert_page()'s "
            "409-conflict retry -- keep working unchanged (verified: all "
            "pre-existing tests pass with zero changes). Applied to all "
            "20 call sites across both clients",
            "commands/jira.py's jira_push() now also catches "
            "requests.HTTPError (alongside the existing JiraConfigError) "
            "around the actual push, printing the now-informative message "
            "and exiting cleanly instead of letting it fall through to a "
            "raw Python traceback -- which is what the user was actually "
            "seeing (their AI agent's own paraphrase of that traceback's "
            "last line was the only detail that ever reached them)",
            "This Node CLI has no Jira/Confluence integration of its own "
            "(scaffolding-only by design) and is unaffected by this fix "
            "beyond the version stamp -- this migration entry exists so "
            "both CLIs report the same sdd_version chain",
            "Verified: cli-python pytest 1163/1163 (1157 unchanged + 6 "
            "new -- a dedicated test_http_errors.py unit-testing the "
            "helper directly, plus one real end-to-end test per client "
            "confirming create_issue()/create_page() now surface the "
            "actual rejection reason); ruff check/format clean; mypy "
            "clean (39 source files, 1 pre-existing unrelated error: "
    ...
    },
    {
        ...
        "notes": [
            "Direct follow-up to the string of Jira push fixes (3.7.5-"
            "3.7.9): those were genuine framework bugs, but the "
            "underlying request -- 'we don't know what all issue will be "
            "there; Jira/Confluence setup is specific to each "
            "organization -- is there a way they can fix it within their "
            "own org, without a framework code change every time?' -- "
            "needed an actual mechanism, not just more one-off fixes. "
            "createmeta is Jira's own source of truth for what a project "
            "+ issue type combination requires, so validating against it "
            "generalizes to any organization's custom issue types "
            "(SAFe Enabler and beyond) without this codebase needing to "
            "know about them in advance",
            "Added JiraClient.get_createmeta_fields() (jira_client.py): "
            "GET .../issue/createmeta?projectKeys=...&issuetypeNames=...&"
            "expand=projects.issuetypes.fields -- the classic endpoint, "
            "the only createmeta option on Server/Data Center; still "
            "functional on Cloud as of writing though Atlassian's docs "
            "mark it deprecated there in favor of a newer two-step API "
            "(flagged as a known follow-up risk, not yet verified against "
            "a live Cloud instance)",
            "Added check_epic_createmeta() (commands/jira.py): resolves "
            "the configured Epic/Feature project+issue type, calls "
            "createmeta, and cross-checks every Jira-required field "
            "against the fixed set feature_extra_fields()/_upsert_issue() "
            "can actually populate (project/issuetype/summary/labels/"
            "description/priority, plus custom_fields.epic_name/team if "
            "configured) -- 'reporter' excluded from the check since "
            "Jira commonly auto-fills it from the API caller even when "
            "marked required. Also flags a non-string description schema "
            "on Server/DC (the 3.7.9 bug class, generalized to catch it "
            "for any future field, not just description)",
            "Wired into `sdd doctor` as a new section, run automatically "
            "whenever integrations.yml configures jira: (silently skipped "
            "otherwise -- an optional adapter, not something every "
            "project has, per this repo's product-scope policy); new "
            "--skip-jira flag opts out even when configured. A finding "
            "failure now factors into doctor's exit code alongside the "
            "existing managed-files drift check",
            "Phase 1 scope, deliberately: Epic/Feature level only (Story/"
            "Task/CHG use the same mechanism and are a natural follow-up, "
            "not a redesign); Confluence has no equivalent schema-"
            "validation API and isn't covered by this",
            "This Node CLI has no doctor/Jira-validation concept of its "
            "own (scaffolding-only by design) and is unaffected by this "
            "beyond the version stamp -- this migration entry exists so "
            "both CLIs report the same sdd_version chain",
            "Verified: cli-python pytest 1191/1191 (1170 unchanged + 21 "
            "new -- unit tests for check_epic_createmeta()'s every "
            "finding type and get_createmeta_fields()'s HTTP shape, plus "
            "CLI-level tests confirming sdd doctor skips/runs/exits "
            "correctly; confirmed the wiring tests fail against the "
            "pre-fix code with an AttributeError on the not-yet-existing "
            "hooks); ruff check/format clean; mypy clean (39 source "
            "files, 1 pre-existing unrelated error); bandit clean (no "
            "new findings)",
        ],
    },
    {
