...
**`/specify-doc data-model`** → `.specify/service/data-model.md` (mvp+, **living/service-level — not per-feature**, see "Living Documents" below) — gate: SRD approved
**`/specify-doc security`** → `.specify/service/security-design.md` (**living/service-level**, see below) — gate: SRD approved
...
feature — they live at `.specify/service/` instead of `.specify/features/{feature}/`,
and are generated once then **extended/amended by every later feature**,
...
`docs/runbook/local-setup.md` and `docs/openapi.yaml` (generated by
`/implement`, not `/specify-doc`) are living artifacts too, for the same
reason — see `implement.prompt.md`.
