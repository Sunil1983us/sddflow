...
Before drafting, a **Feature Size Check** (Step 1.5) looks for signs your
notes actually describe 2+ independently-shippable features rather than
one. If found, it asks whether to build them one at a time — the chosen
feature proceeds as normal, and every other feature's raw notes are
saved to `.specify/contexts/{slug}.raw.md` for a later `/create-context` run.
