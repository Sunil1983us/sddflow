...
/change "scope upgrade from pilot to mvp — add data-model (living, service-level)"
```
...
2. **Detects current stage** from documents that exist — most live at
   ...
   are always **living documents** shared across every feature in this
   service, at `.specify/service/{doc}.md`. `api-spec.md` is also living
   there, but only for `project_type`s that provide an API
   (backend-service-like, fullstack, serverless) — for consumer-only
   types (frontend-spa, mobile) it stays per-feature in `design.md` §3
   instead. See "Living Documents & Cross-Feature Impact" below.
...
| Data | New field, new entity, schema change, payload change | context → data-model (living) → srd → api-spec (living, if this project_type provides an API) → design |
...
`security-design.md` and `data-model.md` are always **living documents** —
shared across every feature in this service, at `.specify/service/{doc}.md`,
not one copy per feature. `api-spec.md` is living there too, for
`project_type`s that provide an API (backend-service-like, fullstack,
serverless); for consumer-only types (frontend-spa, mobile) it stays
per-feature in `design.md` §3 instead. Every other document in the chain
...
Because `security-design.md`, `data-model.md`, and (for API-providing
project types) `api-spec.md` are shared, a CR raised against one feature
can change a unit (an entity, an endpoint, a threat entry) that a
**different** feature also depends on — and that other feature's own
`srd.md`/`design.md` are never read during a normal `/change` walk, since
the walk is scoped to the feature that raised the CR.
...
at `.specify/service/` — and LLD; also api-spec, living too, if this
project_type provides an API).
...
|---|---|
| New field in request/response | api-spec (living if this project_type provides an API — check cross-feature impact; otherwise design.md §3, per-feature) |
| New endpoint | use-cases + srd + design + lld |
| New actor or use case | use-cases + srd |
| New status/state | srd + api-spec + data-model (living — check cross-feature impact) |
| New business rule | srd |
| New DB table/entity | data-model (living — check cross-feature impact) + design + lld |
| NFR change | srd + resilience |
| New integration | srd + design + api-spec (living, if applicable) + lld + analyze |
...
# 2. Run /change
/change "scope upgraded from pilot to mvp — need data-model, api-spec (if applicable), LLD"

# 3. /change will:
#    - ANNOTATE upstream docs (context, brd, use-cases, srd, security §1)
#    - INCORPORATE data-model + api-spec if this is the first feature in
#      the service to need them (or, if a prior feature already created
#      .specify/service/data-model.md / api-spec.md, EXTEND them via the
