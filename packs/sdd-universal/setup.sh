...
detect_project_type() {
  local root="${1:-.}"
  # Fullstack: both backend + frontend indicators
  if [ -f "$root/package.json" ] && { [ -f "$root/pom.xml" ] || [ -f "$root/build.gradle" ] || [ -f "$root/go.mod" ]; }; then
...
content = re.sub(r'project_type:\s*"auto"', 'project_type: "$PROJECT_TYPE"', content)
