...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "git@bitbucket.org:acme-team/widgets.git",
            "bitbucket",
            "acme-team",
            "widgets",
        ),
...
def test_get_provider_dispatches_by_host():
    ...
    assert isinstance(
        get_provider(RemoteInfo("bitbucket", "a", "b")), BitbucketProvider
    )
    ...
    assert isinstance(
        get_provider(RemoteInfo("some-future-host", "a", "b")), UnknownHostProvider
    )
...
def test_github_provider_failure_raises(monkeypatch):
    monkeypatch.setattr(
        "sdd.utils.git_host._run", lambda cmd: (1, "", "not authenticated")
    )
    p = GitHubProvider(RemoteInfo("github", "acme", "widgets"))
...
def test_bitbucket_provider_success(monkeypatch):
    ...
    mock_resp.json.return_value = {
        "links": {
            "html": {"href": "https://bitbucket.org/acme-team/widgets/pull-requests/1"}
        }
    }
    with patch("requests.post", return_value=mock_resp) as post:
        p = BitbucketProvider(RemoteInfo("bitbucket", "acme-team", "widgets"))
...
def test_azure_provider_success(monkeypatch):
    def fake_run(cmd):
        ...
        return (
            0,
            "https://dev.azure.com/acmeorg/AcmeProject/_git/widgets/pullrequest/1",
            "",
        )
